...
class Db {
    ...
    fun <T : Model>insert(model: T) = synchronized(db) { db.collection(model::class.collection()).insertDocument(model.apply { created = Instant.now() }, DocumentCreateOptions().returnNew(true))!!.new!! }
